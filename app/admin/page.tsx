...
import {
  ...
  Instagram, MessageSquare, Monitor, Smartphone, Menu, X
} from "lucide-react"
          ...
          {activeSection === "products" && (
            <div className="max-w-[1400px] mx-auto space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {[
                  { id: "drop-001-comp", name: "Performance Compression Shirt", sku: "DW-001-COMP", price: "39.000 DT", stock: "In Stock", color: "Black", fit: "Slim Fit" },
                  { id: "drop-001-spider", name: "Spider Training Tee", sku: "DW-001-SPIDER", price: "39.000 DT", stock: "In Stock", color: "Black", fit: "Slim Fit" },
                ].map(p => {
                  ...
                  return (
                    <div key={p.id} className="bg-white/[0.02] border border-white/5 rounded-[28px] p-8 flex gap-6 items-start hover:border-white/10 transition-all">
                      <div className="size-24 bg-white/5 border border-white/10 rounded-2xl overflow-hidden shrink-0 flex items-center justify-center">
                        <Image src="/images/product-main.png" alt={p.name} width={96} height={96} className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="text-[10px] font-black text-[var(--accent)] uppercase tracking-[3px] mb-1">{p.sku}</div>
                        <h3 className="text-[16px] font-black uppercase leading-tight mb-3">{p.name}</h3>
                        <div className="flex flex-wrap gap-2 mb-4">
                          <span className="px-3 py-1 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[9px] font-black uppercase tracking-wider rounded-full">{p.stock}</span>
